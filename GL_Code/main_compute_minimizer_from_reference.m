...
% - reference solution from "main_compute_minimizer_level4.m" required
...
ht_level = 5; % mesh size level A (P2)
h_ref_level = 9;
ht_ref_level = 7;
ell = 10; % oversampling of ell-layers for u (LOD)

tol = 10^(-12); % tolerance for termination
i_max = 100000; % maximum number of iterations

save_path = "solution_kappa"+kappa+"_H"+H_level+"_ht"+ht_level+".mat"; % path for save file
...
path_best = "solution_level4_kappa"+kappa+".mat";
u_h = getBestApproximation_for_u(path_best,x_a,x_b,kappa,Stiff_h,M_h,h_level,h_ref_level);
A_ht = getBestApproximation_for_A(path_best,x_a,x_b,ht_level,ht_ref_level,Bdx_ht,Bdy_ht,M_A,S_curl,S_div);
...
%% postprocessing
% get meshes
[T_h,~,P1,~] = getCoarseFineTriangulation(x_a,x_b,h_level,h_ref_level);
[T_ht,T_h_ref,~,Pt0] = getCoarseFineTriangulation(x_a,x_b,ht_level,h_ref_level);
[~,T_h_ref_P2,~] = getCoarseFineTriangulation_for_P2(T_ht,T_h_ref,Pt0);
P1 = P1';

[nodes2mesh_h_ref,nodes2mesh_hx_ref,nodes2mesh_hy_ref] = getNodes2MeshP1P2(T_h_ref.p,T_h_ref_P2.p,x_a,x_b,boundary_u,boundary_A);
[Bx_h_ref,By_h_ref] = getBoundaryNodes_for_A(T_h_ref_P2.p,x_a,x_b,boundary_A);
Bdx_h_ref = getBoundaryRestriction(Bx_h_ref);
Bdy_h_ref = getBoundaryRestriction(By_h_ref);

% get meshes
[T_ht,T_ht_ref,Pt1,Pt0] = getCoarseFineTriangulation(x_a,x_b,ht_level,ht_ref_level);
[T_ht_P2,T_ht_ref_P2,P2] = getCoarseFineTriangulation_for_P2(T_ht,T_ht_ref,Pt0);

[nodes2mesh_ht_ref,nodes2mesh_htx_ref,nodes2mesh_hty_ref] = getNodes2MeshP1P2(T_ht_ref.p,T_ht_ref_P2.p,x_a,x_b,boundary_u,boundary_A);
[Bx_ht_ref,By_ht_ref] = getBoundaryNodes_for_A(T_ht_ref_P2.p,x_a,x_b,boundary_A);
Bdx_ht_ref = getBoundaryRestriction(Bx_ht_ref);
Bdy_ht_ref = getBoundaryRestriction(By_ht_ref);

M_h_ref = assemble_mass_matrix(T_h_ref.t,T_h_ref.p,nodes2mesh_h_ref);i
Stiff_h_ref = assemble_stiffness_matrix(T_h_ref.t,T_h_ref.p,nodes2mesh_h_ref);

S_curl_ht_ref = assemble_curl_P2(T_ht_ref_P2.t,T_ht_ref_P2.p,nodes2mesh_htx_ref,nodes2mesh_hty_ref);
S_div_ht_ref = assemble_div_P2(T_ht_ref_P2.t,T_ht_ref_P2.p,nodes2mesh_htx_ref,nodes2mesh_hty_ref);
M_A_ht_ref = assemble_mass_A_P2(T_ht_ref_P2.t,T_ht_ref_P2.p,nodes2mesh_htx_ref,nodes2mesh_hty_ref);

% polongate solution to reference mesh
A_sol = get_A_on_u_mesh(A_ht,ht_ref_level,ht_level,P2,M_A,M_A_ht_ref,Bdx_ht_ref,Bdy_ht_ref,Bdx_ht,Bdy_ht,nodes2mesh_htx);
u_sol = get_u_on_A_mesh(u_h,h_level,h_ref_level,P1',M_h,M_h_ref);
...
error_u_H1 = sqrt(real(e_u'*(Stiff_h_ref + M_h_ref)*e_u));
error_u_L2 = sqrt(real(e_u'*(Stiff_h_ref + M_h_ref)*e_u));
error_u_H1kappa = sqrt(real(e_u'*((1/kappa^2)*Stiff_h_ref + M_h_ref)*e_u));

error_A_H1 = sqrt(real(e_A'*(S_curl_ht_ref + S_div_ht_ref + M_A_ht_ref)*e_A));
error_A_L2 = sqrt(real(e_A'*(M_A_ht_ref)*e_A));
error = sqrt(error_u_H1kappa^2 + error_A_H1^2);

%% save
clearvars -except H_level ht_level h_level h_ref_level kappa kappa_str error_u_H1 error_u_L2 error_u_H1kappa error_A_L2 error_A_H1 error counter A_h A_ht u_h u_ht u_LOD E Q H_mag ell tol beta update_para1 update_para2 save_path  parallel
save(save_path,'-v7.3')
