...
for k = my_index_start:my_index_end
    ...
    for i = 1:6
        index_i = nodes2mesh_x(tri(i));
        if index_i ~= 0
            F_worker(index_i) = F_worker(index_i) + detBT*evaluateQuadrature(transpose(f1_in_quad.*phi_P2_in_quad(:,i)),w);
        end

        index_i = nodes2mesh_y(tri(i));
        if index_i ~= 0
            F_worker(Nx + index_i) = F_worker(Nx + index_i) + detBT*evaluateQuadrature(transpose(f2_in_quad.*phi_P2_in_quad(:,i)),w);
        end
    end
end
