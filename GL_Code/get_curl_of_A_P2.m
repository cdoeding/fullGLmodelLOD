function [curlA,E_curl] = get_curl_of_A_P2(H,A,T,Nd,nodes2mesh_x,nodes2mesh_y,parallel)

if parallel
    [curlA,E_curl] = get_curl_of_A_P2_parallel(H,A,T,Nd,nodes2mesh_x,nodes2mesh_y);
else
    [curlA,E_curl] = get_curl_of_A_P2_sequential(H,A,T,Nd,nodes2mesh_x,nodes2mesh_y);
end
end
...
function [curlA,E_curl] = get_curl_of_A_P2_sequential(H,A,T,Nd,nodes2mesh_x,nodes2mesh_y)
...
function [curlA,E_curl] = get_curl_of_A_P2_parallel(H,A,T,Nd,nodes2mesh_x,nodes2mesh_y)
% sequential implementation
Nx = sum(logical(nodes2mesh_x));
...
spmd
    ...
    for k = my_index_start:my_index_end
        ...
        curlA_worker(k) = curlA_in_quad(1);
        E_curl_worker = E_curl_worker + detBT*evaluateQuadrature((curlA_in_quad - H_in_quad).^2,w);

    end
end

% collect results from workers
curlA = curlA_worker{1};
E_curl = E_curl_worker{1};
for j = 2:length(my_index_start)
    curlA = curlA + curlA_worker{j};
    E_curl = E_curl + E_curl_worker{j};
end
